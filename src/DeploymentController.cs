...
static class DeploymentController
{
    ...
    public static void HandleDeploymentInput()
    {
		...
		if (SwinGame.KeyTyped(KeyCode.vk_b)) // new feature 
		{
        ...
        if (SwinGame.MouseClicked(MouseButton.LeftButton)){
            ...
            //bug found for the direction of the ship
            if (GameController.HumanPlayer.ReadyToDeploy & UtilityFunctions.IsMouseInRectangle(PLAY_BUTTON_LEFT, TOP_BUTTONS_TOP, PLAY_BUTTON_WIDTH, TOP_BUTTONS_HEIGHT)) {
                GameController.EndDeployment();
            } else if (UtilityFunctions.IsMouseInRectangle(UP_DOWN_BUTTON_LEFT, TOP_BUTTONS_TOP, DIR_BUTTONS_WIDTH, TOP_BUTTONS_HEIGHT)) {
                _currentDirection = Direction.UpDown; //bug was here
            } else if (UtilityFunctions.IsMouseInRectangle(LEFT_RIGHT_BUTTON_LEFT, TOP_BUTTONS_TOP, DIR_BUTTONS_WIDTH, TOP_BUTTONS_HEIGHT)) {
                _currentDirection = Direction.LeftRight;
            } else if (UtilityFunctions.IsMouseInRectangle(RANDOM_BUTTON_LEFT, TOP_BUTTONS_TOP, RANDOM_BUTTON_WIDTH, TOP_BUTTONS_HEIGHT)) {
                GameController.HumanPlayer.RandomizeDeployment();
            } else if (UtilityFunctions.IsMouseInRectangle(LEFT_RIGHT_BUTTON_LEFT-200, TOP_BUTTONS_TOP, RANDOM_BUTTON_WIDTH, TOP_BUTTONS_HEIGHT)) {
                GameController.AddNewState(GameState.ViewingMainMenu); // new features check when player press the button
			}
    ...
    private static void DoDeployClick()
    {
        ...
        //issues with the row calculation
        int row = 0;
        int col = 0;
        row = Convert.ToInt32(Math.Floor((mouse.Y - UtilityFunctions.FIELD_TOP) / (UtilityFunctions.CELL_HEIGHT + UtilityFunctions.CELL_GAP)));//bug found here
    ...
    public static void DrawDeployment()
    {
		...
		// new feature drawing the button
		 SwinGame.DrawBitmap(GameResources.GameImage("BackButton"), LEFT_RIGHT_BUTTON_LEFT-200, TOP_BUTTONS_TOP); 
        //DrawShips
