    ...
    static class DeploymentController
    {
        ...
        public static void HandleDeploymentInput()
        {
            ...
            if (SwinGame.MouseClicked(MouseButton.LeftButton))
            {
                ...
                if ((int)selected != (int)ShipName.None)
        ...
        private static void DoDeployClick()
        {
            ...
            int row, col;
            row = Convert.ToInt32(Math.Floor((double)(mouse.Y) / (float)(UtilityFunctions.CELL_HEIGHT + UtilityFunctions.CELL_GAP)));
        ...
        public static void DrawDeployment()
        {
            ...
            if ((int)_currentDirection == (int)Direction.LeftRight)
                SwinGame.DrawBitmap(GameResources.GameImage("LeftRightButton"), (float)LEFT_RIGHT_BUTTON_LEFT, (float)TOP_BUTTONS_TOP);
            else
                SwinGame.DrawBitmap(GameResources.GameImage("UpDownButton"), (float)LEFT_RIGHT_BUTTON_LEFT, (float)TOP_BUTTONS_TOP);
            ...
            foreach (ShipName sn in Enum.GetValues(typeof(ShipName)))
            {
                ...
                if (i >= 0)
                {
                    if ((int)sn == (int)_selectedShip)
                        SwinGame.DrawBitmap(GameResources.GameImage("SelectedShip"), (float)SHIPS_LEFT, (float)SHIPS_TOP + (i * SHIPS_HEIGHT));
                }
            }

            if (GameController.HumanPlayer.ReadyToDeploy)
                SwinGame.DrawBitmap(GameResources.GameImage("PlayButton"), (float)PLAY_BUTTON_LEFT, (float)TOP_BUTTONS_TOP);

            SwinGame.DrawBitmap(GameResources.GameImage("RandomButton"), (float)RANDOM_BUTTON_LEFT, (float)TOP_BUTTONS_TOP);
